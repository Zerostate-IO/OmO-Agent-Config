...
test.describe('OmO Agent Config UI', () => {
  ...
  // ===========================================
  // Fallback Editor E2E Tests
  // ===========================================

  test('Fallback editor button visible in agent detail modal', async ({ page }) => {
    // Wait for agents to load
    await page.waitForTimeout(2000);

    // Click View Details button to open detail modal
    const sisyphusCard = await page.locator('.agent-config-card[data-agent-name="sisyphus"]');
    ...
    // Verify fallback chain section exists (button presence implies section exists)
    const fallbackHeader = await page.locator('.fallback-chain-header');
    await expect(fallbackHeader).toBeVisible();

    // Verify edit button is visible
    const editBtn = await page.locator('#edit-fallback-priority');
    await expect(editBtn).toBeVisible();

    // Take screenshot
    await page.screenshot({ path: 'test-results/fallback-editor-button-visible.png' });
  });

  test('Fallback editor modal loads and displays chain entries', async ({ page, context }) => {
    const testPage = await context.newPage();

    // Stub fallbacks API with fixture
    await testPage.route('**/api/agents/*/fallbacks*', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
          ...
          fallbackChain: [
            ...
            { providers: ['google'], model: 'gemini-3-pro-high' }
          ]
        })
      });
    });
    ...
    // Open agent detail modal by clicking View Details
    await testPage.locator('.agent-config-card[data-agent-name="sisyphus"] button:has-text("View Details")').click();
    await testPage.waitForTimeout(500);

    // Click edit fallback priority button
    await testPage.locator('#edit-fallback-priority').click();
    await testPage.waitForTimeout(500);

    // Verify fallback editor modal content
    const fallbackEditor = await testPage.locator('.fallback-editor');
    await expect(fallbackEditor).toBeVisible();

    // Verify source badge shows 'upstream'
    const sourceBadge = await testPage.locator('.source-value.upstream');
    await expect(sourceBadge).toBeVisible();

    // Verify entries list has 2 entries
    const entries = await testPage.locator('[data-testid^="fallback-entry-"]');
    const count = await entries.count();
    expect(count).toBe(2);

    // Verify first entry has correct model
    const model0 = await testPage.locator('[data-testid="fallback-model-0"]');
    await expect(model0).toHaveValue('claude-opus-4-6');

    // Verify first entry has correct providers
    const providers0 = await testPage.locator('[data-testid="fallback-providers-0"]');
    await expect(providers0).toHaveValue('anthropic');

    // Verify second entry has correct model
    const model1 = await testPage.locator('[data-testid="fallback-model-1"]');
    await expect(model1).toHaveValue('gemini-3-pro-high');

    // Take screenshot
    await testPage.screenshot({ path: 'test-results/fallback-editor-modal-entries.png' });
    await testPage.close();
  });

  test('Fallback editor reorder entries works', async ({ page, context }) => {
    const testPage = await context.newPage();
    ...
    // Open fallback editor by clicking View Details
    await testPage.locator('.agent-config-card[data-agent-name="sisyphus"] button:has-text("View Details")').click();
    await testPage.waitForTimeout(500);
    await testPage.locator('#edit-fallback-priority').click();
    await testPage.waitForTimeout(500);
    ...
    await expect(testPage.locator('[data-testid="fallback-model-0"]')).toHaveValue('claude-opus-4-6');
    await expect(testPage.locator('[data-testid="fallback-model-1"]')).toHaveValue('gemini-3-pro-high');

    // Move second entry up
    await testPage.locator('[data-testid="fallback-move-up-1"]').click();
    await testPage.waitForTimeout(300);

    // Verify order changed
    await expect(testPage.locator('[data-testid="fallback-model-0"]')).toHaveValue('gemini-3-pro-high');
    await expect(testPage.locator('[data-testid="fallback-model-1"]')).toHaveValue('claude-opus-4-6');

    // Move first entry back down
    await testPage.locator('[data-testid="fallback-move-down-0"]').click();
    await testPage.waitForTimeout(300);
    ...
    // Verify first entry's move-up is disabled
    await expect(testPage.locator('[data-testid="fallback-move-up-0"]')).toBeDisabled();

    // Verify last entry's move-down is disabled
    await expect(testPage.locator('[data-testid="fallback-move-down-2"]')).toBeDisabled();

    await testPage.screenshot({ path: 'test-results/fallback-editor-reorder.png' });
    await testPage.close();
  });

  test('Fallback editor add/remove entry controls work', async ({ page, context }) => {
    const testPage = await context.newPage();
    ...
    // Verify 1 entry initially
    let entries = await testPage.locator('[data-testid^="fallback-entry-"]');
    expect(await entries.count()).toBe(1);

    // Click add entry button
    await testPage.locator('#fallback-add-entry').click();
    await testPage.waitForTimeout(300);

    // Verify 2 entries now
    entries = await testPage.locator('[data-testid^="fallback-entry-"]');
    expect(await entries.count()).toBe(2);

    // Verify new entry is empty and has validation error (new entries pre-validated as invalid)
    const model1 = await testPage.locator('[data-testid="fallback-model-1"]');
    await expect(model1).toHaveValue('');

    // Verify save button is disabled (new entry has validation error)
    await expect(testPage.locator('#fallback-save')).toBeDisabled();

    // Remove the new entry
    await testPage.locator('[data-testid="fallback-remove-1"]').click();
    await testPage.waitForTimeout(300);

    // Verify back to 1 entry
    entries = await testPage.locator('[data-testid^="fallback-entry-"]');
    expect(await entries.count()).toBe(1);

    // Verify save button is enabled again
    await expect(testPage.locator('#fallback-save')).toBeEnabled();

    await testPage.screenshot({ path: 'test-results/fallback-editor-add-remove.png' });
    await testPage.close();
  });

  test('Fallback editor save sends correct payload shape', async ({ page, context }) => {
    const testPage = await context.newPage();
    let capturedPayload = null;

    // Stub fallbacks API - capture PUT payload
    await testPage.route('**/api/agents/*/fallbacks*', async (route) => {
      ...
      if (method === 'GET') {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            agent: 'sisyphus',
            source: 'override',
            fallbackChain: capturedPayload.fallbackChain
          })
        });
      } else {
        await route.continue();
    ...
    // Modify first entry
    await testPage.locator('[data-testid="fallback-model-0"]').fill('gpt-5.2');
    await testPage.locator('[data-testid="fallback-providers-0"]').fill('openai, anthropic');
    await testPage.locator('[data-testid="fallback-variant-0"]').fill('high');
    await testPage.waitForTimeout(300);

    // Click save
    await testPage.locator('#fallback-save').click();
    await testPage.waitForTimeout(500);

    // Verify payload was captured
    expect(capturedPayload).not.toBeNull();
    expect(capturedPayload.fallbackChain).toBeDefined();
    expect(capturedPayload.fallbackChain.length).toBe(1);
    
    // Verify payload shape
    const entry = capturedPayload.fallbackChain[0];
    expect(entry.model).toBe('gpt-5.2');
    expect(entry.providers).toEqual(['openai', 'anthropic']); // Should be trimmed array
    expect(entry.variant).toBe('high');

    await testPage.screenshot({ path: 'test-results/fallback-editor-save-payload.png' });
    await testPage.close();
  });

  test('Fallback editor reset triggers DELETE call', async ({ page, context }) => {
    const testPage = await context.newPage();
    let deleteCalled = false;

    // Stub fallbacks API - track DELETE call
    await testPage.route('**/api/agents/*/fallbacks*', async (route) => {
      const method = route.request().method();
    ...
    // Verify source badge shows 'override'
    await expect(testPage.locator('.source-value.override')).toBeVisible();

    // Click reset button
    await testPage.locator('#fallback-reset-upstream').click();
    await testPage.waitForTimeout(500);

    // Verify DELETE was called
    expect(deleteCalled).toBe(true);

    await testPage.screenshot({ path: 'test-results/fallback-editor-reset-delete.png' });
    await testPage.close();
  });

  test('Fallback editor modal load failure shows error', async ({ page, context }) => {
    const testPage = await context.newPage();

    // Stub fallbacks API to return error
    await testPage.route('**/api/agents/*/fallbacks*', async (route) => {
      await route.fulfill({
        status: 500,
        contentType: 'application/json',
    ...
    const errorContainer = await testPage.locator('.fallback-error');
    await expect(errorContainer).toBeVisible();

    // Verify error message contains 'error'
    await expect(errorContainer).toContainText(/error/i);

    await testPage.screenshot({ path: 'test-results/fallback-editor-load-error.png' });
    await testPage.close();
  });
