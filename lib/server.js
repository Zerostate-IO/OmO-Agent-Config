...
const { getModels, filterModels, formatModel, sortModels, findDuplicateModels, getBestProvider } = require('./core/models');
const { CONFIG_FILE, CONFIGS_DIR, CACHE_DIR, BACKUP_DIR, ACTIVE_CONFIG_FILE, AGENT_PROFILES, loadJsoncFile } = require('./constants');
...
const routes = {
  ...
  'GET /api/agents': async (req, res) => {
    try {
      ...
      const agents = await getAllAgentDocumentation(models);
      
      sendJSON(res, {
        agents: agents.map(agent => {
          const profile = AGENT_PROFILES[agent.name] || null;
  ...
  'GET /api/agents/:name': async (req, res, name) => {
    try {
      ...
      const { models } = await getModels();
      const agent = await getAgentDocumentation(name, models);
