...
const routes = {
  ...
  'POST /api/providers': async (req, res) => {
    try {
      ...
      const { getProviderPolicies, saveProviderPolicies: savePoliciesOverride, PROVIDER_POLICIES_DEFAULTS } = require('./constants');
      
      // Get current policies
      const currentData = getProviderPolicies();
      const mergedProviders = { ...currentData.providers };
      ...
      const overrideFile = path.join(process.env.HOME || process.env.USERPROFILE, '.config/opencode/provider-policies.json');
      ...
      // Invalidate cache in constants.js
      const { invalidateProviderPoliciesCache } = require('./constants');
      invalidateProviderPoliciesCache();
      
      sendJSON(res, {
        success: true,
        message: 'Provider policies saved successfully',
        providers: mergedProviders
      });
    } catch (error) {
