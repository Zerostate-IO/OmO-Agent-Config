...
const { getModels, filterModels, formatModel, sortModels, findDuplicateModels, getDuplicateKey, getBestProvider, rankProvider } = require('./core/models');
const { CONFIG_FILE, CONFIGS_DIR, CACHE_DIR, BACKUP_DIR, ACTIVE_CONFIG_FILE, AGENT_PROFILES, normalizeAgentKey, loadJsoncFile } = require('./constants');
const { normalizeFallbackModels, normalizeFallbackModelsRich } = require('./core/fallback-models');
const { createBackup, listBackups, restoreBackup, deleteBackup, purgeBackups } = require('./core/backup');
const { buildProviderDiagnostics, buildProviderHealthCheck, buildConfigSplitDiagnostics } = require('./core/provider-diagnostics');
...
function isSameOriginHeader(req, headerName) {
...
function validateProviderHealthRequest(req, body) {
  if (!isJsonRequest(req)) {
    throw Object.assign(new Error('Provider health check requires Content-Type: application/json'), { statusCode: 415 });
  }
  if (!isSameOriginHeader(req, 'origin') || !isSameOriginHeader(req, 'referer')) {
    throw Object.assign(new Error('Provider health check requires same-origin request headers'), { statusCode: 403 });
  }
  if (body.live !== undefined && typeof body.live !== 'boolean') {
    throw Object.assign(new Error('Invalid option: live must be a boolean'), { statusCode: 400 });
  }
  if (body.refreshModels !== undefined && typeof body.refreshModels !== 'boolean') {
    throw Object.assign(new Error('Invalid option: refreshModels must be a boolean'), { statusCode: 400 });
  }
  if (body.providers !== undefined) {
    if (!Array.isArray(body.providers) || body.providers.some(provider => typeof provider !== 'string')) {
      throw Object.assign(new Error('Invalid option: providers must be an array of strings'), { statusCode: 400 });
    }
  }
}

function buildProviderHealthOptions(body) {
  const live = body.live === true;
  const refreshModels = body.refreshModels === true;
  const timeoutMs = body.timeoutMs === undefined ? 15000 : Number(body.timeoutMs);
  const providers = body.providers === undefined ? undefined : body.providers;
  return { live, refreshModels, providers, timeoutMs };
}
...
const routes = {
  ...
  'GET /api/providers/diagnostics': async (req, res) => {
    try {
      ...
      const diagnostics = await buildProviderDiagnostics({ refresh });
  ...
  'POST /api/providers/health-check': async (req, res) => {
    try {
      const body = await parseBody(req);
      validateProviderHealthRequest(req, body);
      const opts = buildProviderHealthOptions(body);

      const health = await buildProviderHealthCheck(opts);

      sendJSON(res, health);
    } catch (error) {
      const status = error.statusCode || (error.message && error.message.startsWith('Invalid option:') ? 400 : 500);
      console.error('Error building provider health check:', error);
      sendError(res, error.message, status);
    }
  },
  // GET /api/config-split-diagnostics - Get config split and rename diagnostics (compat alias)
  'GET /api/config-split-diagnostics': async (req, res) => {
    try {
      const diagnostics = buildConfigSplitDiagnostics();
      sendJSON(res, diagnostics);
    } catch (error) {
      console.error('Error building config split diagnostics:', error);
      sendError(res, error.message, 500);
    }
  },
  // GET /api/config/diagnostics - Canonical config diagnostics (config split + rename state)
  'GET /api/config/diagnostics': async (req, res) => {
    try {
      const diagnostics = buildConfigSplitDiagnostics();
      sendJSON(res, diagnostics);
    } catch (error) {
      console.error('Error building config diagnostics:', error);
      sendError(res, error.message, 500);
    }
  },
  ...
  'GET /api/agents': async (req, res) => {
    try {
      ...
      // Load active config to get configured fallback_models per agent
      const activeConfig = loadJsoncFile(CONFIG_FILE) || {};
      const agentConfigs = activeConfig.agents || {};
      ...
      sendJSON(res, {
        agents: agents.map(agent => {
          ...
          const agentConfig = agentConfigs[normalizedName] || {};
          const configuredFallbackModels = normalizeFallbackModels(agentConfig.fallback_models);
          const configuredFallbacksRaw = normalizeFallbackModelsRich(agentConfig.fallback_models);
  ...
  'GET /api/agents/:name': async (req, res, name) => {
    try {
      ...
      // Load active config to get configured fallback_models for this agent
      const activeConfig = loadJsoncFile(CONFIG_FILE) || {};
      const agentConfigs = activeConfig.agents || {};
      const agentConfig = agentConfigs[normalizedName] || {};
      const configuredFallbackModels = normalizeFallbackModels(agentConfig.fallback_models);
  ...
  // GET /api/upstream/drift - Check for upstream drift
  'GET /api/upstream/drift': async (req, res) => {
    try {
      const { execSync } = require('child_process');
      const driftCheckPath = path.join(__dirname, '..', 'scripts', 'drift-check.js');

      let driftData;
      try {
        const output = execSync(`node "${driftCheckPath}" --json`, {
          encoding: 'utf8',
          timeout: 30000
        });
        driftData = JSON.parse(output);
      } catch (e) {
        // If drift-check fails, return a graceful error
        sendJSON(res, {
          hasDrift: false,
          error: 'Could not check for drift: ' + e.message,
      ...
      sendJSON(res, {
        hasDrift: driftData.hasDrift,
        newAgents: driftData.newAgents || [],
        missingAgents: driftData.missingAgents || [],
        changedAgents: driftData.changedAgents || [],
        newCategories: driftData.newCategories || [],
        missingCategories: driftData.missingCategories || [],
        changedCategories: driftData.changedCategories || [],
        actionRequired: driftData.actionRequired || [],
        pinnedSha: driftData.pinnedSha,
        currentSha: driftData.currentSha,
        upstreamResolved: driftData.upstreamResolved !== false,
        error: driftData.error || null
      });
    } catch (error) {
      console.error('Error checking upstream drift:', error);
      sendError(res, error.message, 500);
    }
  },

  // POST /api/upstream/sync - Sync model requirements with upstream
  'POST /api/upstream/sync': async (req, res) => {
    try {
      const body = await parseBody(req);
      const applyRequested = body.dryRun === false;

      if (applyRequested) {
        sendJSON(res, {
          success: false,
          dryRun: false,
          error: 'Apply mode is disabled; run drift/sync manually after review. Source file writes require backup/write-lock guard not yet implemented.',
          changedAgents: [],
          changedCategories: [],
          warnings: ['Apply mode disabled — no source files modified'],
          sourceRef: { commitSha: null },
          summary: 'Apply mode disabled'
        }, 501);
        return;
      }

      const { execSync } = require('child_process');
      const syncScriptPath = path.join(__dirname, '..', 'scripts', 'provider-aware-sync.js');

      if (!fs.existsSync(syncScriptPath)) {
        sendJSON(res, {
          success: false,
          dryRun: true,
          error: 'Sync script not found: ' + syncScriptPath,
          changedAgents: [],
          changedCategories: [],
          warnings: ['provider-aware-sync.js is missing from scripts/ directory'],
          sourceRef: { commitSha: null }
        }, 500);
        return;
      }
      ...
      let rawOutput;
      try {
        rawOutput = execSync(`node "${syncScriptPath}" ${scriptArgs.join(' ')}`, {
          encoding: 'utf8',
          timeout: 60000
        });
      } catch (e) {
        ...
        let parsed = null;
        try { parsed = JSON.parse(stdout); } catch (pe) { /* not JSON */ }

        if (parsed && parsed.success === false) {
          sendJSON(res, {
            success: false,
            dryRun: true,
            error: parsed.error || 'Sync script reported failure',
            changedAgents: parsed.changedAgents || [],
            changedCategories: parsed.changedCategories || [],
            warnings: parsed.warnings || [],
            sourceRef: parsed.sourceRef || { commitSha: null }
          }, 500);
          return;
        }
        ...
        sendJSON(res, {
          success: false,
          dryRun: true,
          error: 'Sync script failed: ' + (e.message || 'unknown error'),
          changedAgents: [],
          changedCategories: [],
          warnings: ['Script exited with code ' + (e.status || 'unknown')],
          sourceRef: { commitSha: null }
        }, 500);
        return;
      }
      ...
      let scriptResult;
      try {
        scriptResult = JSON.parse(rawOutput);
      } catch (e) {
        sendJSON(res, {
          success: false,
          dryRun: true,
          error: 'Could not parse sync script output as JSON',
          changedAgents: [],
          changedCategories: [],
          warnings: ['Unparseable script output'],
          sourceRef: { commitSha: null }
        }, 500);
        return;
      }

      if (!scriptResult.sourceRef) {
        scriptResult.sourceRef = { commitSha: null };
      }

      const summary = scriptResult.summary || [
        scriptResult.changedAgents?.length || 0, 'agent(s),',
        scriptResult.changedCategories?.length || 0, 'category(ies)'
      ].join(' ');

      sendJSON(res, {
        success: scriptResult.success !== false,
        dryRun: scriptResult.dryRun !== false,
        sourceRef: {
          commitSha: scriptResult.sourceRef?.pinnedSha || scriptResult.sourceRef?.commitSha || null,
          modelRequirementsFile: scriptResult.sourceRef?.modelRequirementsFile || null
        },
        changedAgents: scriptResult.changedAgents || [],
        changedCategories: scriptResult.changedCategories || [],
        warnings: scriptResult.warnings || [],
        summary,
        message: scriptResult.message || null
      });
    } catch (error) {
