...
function parseModels(output) {
  ...
  let skippedCount = 0;
  let lineNumber = 0;
  let nestedPathCount = 0;
  let providerMismatchCount = 0;

  const modelHeaderLineRe = /^[a-z0-9_.-]+\/[a-z0-9_.:\/-]+$/i;

  for (const line of lines) {
    lineNumber++;
    
    const trimmedLine = line.trim();
    if (modelHeaderLineRe.test(trimmedLine) && braceCount === 0) {
      currentModel = trimmedLine;
      jsonBuffer = '';
      ...
      if (braceCount === 0 && jsonBuffer) {
        try {
          ...
          // Check for nested model path (more than one slash)
          const headerSegments = currentModel.split('/');
          if (headerSegments.length > 2) {
            nestedPathCount++;
            warnings.push(`Line ${lineNumber}: Model "${currentModel}" has nested provider/model path (${headerSegments.length - 1} segments)`);
          }
          
          // Check for providerID consistency with header prefix
          const headerProvider = headerSegments[0];
          if (modelData.providerID && modelData.providerID !== headerProvider) {
            providerMismatchCount++;
            warnings.push(`Line ${lineNumber}: Model "${currentModel}" has providerID "${modelData.providerID}" but header prefix is "${headerProvider}"`);
          }
          ...
          models.push({
            ...
            modelID: baseId,
            id: currentModel,
            _nestedPath: headerSegments.length > 2
          });
          parsedCount++;
        } catch (e) {
  ...
  // Summary for nested paths and provider mismatches (informational)
  if (nestedPathCount > 0) {
    warnings.push(`${nestedPathCount} model(s) have nested provider/model paths (e.g., fireworks-ai/accounts/fireworks/models/xxx)`);
  }
  if (providerMismatchCount > 0) {
    warnings.push(`${providerMismatchCount} model(s) have providerID field differing from header prefix`);
  }
  ...
  try {
    ...
    if (models.length === 0 && errors.length > 0 && allowCachedFallback) {
      ...
      if (cached && cached.models && cached.models.length > 0) {
        const cachedDate = cached.timestamp ? new Date(cached.timestamp).toISOString() : 'unknown';
        return {
          ...
          warnings: [
            ...warnings,
            'Parse failed with 0 models - using cached models as fallback',
            `CLI errors: ${errors.join('; ')}`,
            `Fallback source: ${cached.models.length} models cached at ${cachedDate}`
          ],
          errors,
          partial: true,
          _fallbackReason: 'parse_failure'
        };
      }
    }
    ...
    if (allowCachedFallback) {
      ...
      if (cached && cached.models && cached.models.length > 0) {
        ...
        return {
          ...
          fromCache: true,
          warnings: [
            'CLI execution failed - using cached models as fallback',
            `CLI error: ${execError.message}`,
            `Fallback source: ${cached.models.length} models cached at ${cachedDate}`
          ],
          errors: [`CLI error: ${execError.message}`],
          partial: true,
          _fallbackReason: 'cli_execution_failure'
        };
      }
    }
...
/**
 * Generate a duplicate key for a model
 * Used to identify the same model from different providers
 */
