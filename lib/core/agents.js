...
const { CACHE_DIR, PROVIDER_ALIASES, getProviderAliases, getAgentFallbackOverrides } = require('../constants');
...
function getRecommendedModels(metadata, availableModels, limit = 5) {
  ...
  // Get effective fallback chain (merge override + upstream)
  const overrides = getAgentFallbackOverrides();
  const effectiveChain = getEffectiveFallbackChain(agentKey, overrides);

  // If agent has upstream requirements, use fallback chain resolution
  if (requirements && effectiveChain) {
    // Build effective requirements with the merged chain
    const effectiveRequirements = { ...requirements, fallbackChain: effectiveChain };
    const availability = buildAvailabilityMap(availableModels);

    // Check gating conditions using shared helper
    const gatingResult = checkGatingConditions(effectiveRequirements, availability);
