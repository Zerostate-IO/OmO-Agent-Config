...
async function viewAgentDetail(agentName) {
    try {
        ...
        if (fullAgent.fallbackChain && fullAgent.fallbackChain.length > 0) {
            content += '<div class="agent-section">';
            content += '<div class="fallback-chain-header">';
            content += '<h4>Fallback Chain</h4>';
            content += '<button id="edit-fallback-priority" class="btn btn-secondary btn-sm" onclick="openAgentFallbackEditor(\'' + agentName + '\')">Edit Fallback Priority</button>';
            content += '</div>';
            content += '<p>If the primary model is unavailable, this agent will try:</p>';
            content += '<ol>';
            fullAgent.fallbackChain.forEach(entry => {
                const providerStr = entry.providers ? entry.providers.join(', ') : 'any';
                const modelStr = entry.model || 'any';
                const variantStr = entry.variant ? ' (' + entry.variant + ')' : '';
                content += '<li>' + providerStr + ' / ' + modelStr + variantStr + '</li>';
            });
            content += '</div>';
        }
...
// Expose diagnostics functions to window
window.openProviderDiagnostics = openProviderDiagnostics;
window.showProviderDiagnosticsModal = showProviderDiagnosticsModal;
window.refreshModelsCache = refreshModelsCache;

// ===========================================
// Fallback Priority Editor
// ===========================================

/**
 * Open the fallback editor modal for a specific agent
 */
async function openAgentFallbackEditor(agentName) {
    state.fallbackEditor = {
        agentName: agentName,
        entries: [],
        source: 'upstream',
        validationErrors: {},
    ...
    content += '<div class="fallback-loading">Loading fallback chain...</div>';
    content += '</div>';
    showModal('Edit Fallback Priority: ' + agentName, content);
    
    try {
        await loadAgentFallbackChain(agentName);
        renderFallbackEditorModal();
    } catch (error) {
        console.error('Failed to load fallback chain:', error);
        const errorContent = '<div class="fallback-editor"><div class="fallback-error">Failed to load fallback chain: ' + escapeHtml(error.message) + '</div></div>';
        elements.modalBody.innerHTML = errorContent;
    }
}

/**
 * Load fallback chain from API for the agent
 */
async function loadAgentFallbackChain(agentName) {
    const response = await fetch('/api/agents/' + encodeURIComponent(agentName) + '/fallbacks');
    
    if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to load fallbacks');
    }
    
    const data = await response.json();
    
    state.fallbackEditor.agentName = data.agent;
    state.fallbackEditor.source = data.source || 'upstream';
    state.fallbackEditor.entries = (data.fallbackChain || []).map(function(entry, index) {
        return {
            model: entry.model || '',
            providers: Array.isArray(entry.providers) ? entry.providers.join(', ') : '',
            variant: entry.variant || ''
        };
    });
    state.fallbackEditor.isLoading = false;
    state.fallbackEditor.hasChanges = false;
}

/**
 * Render the fallback editor modal content
 */
function renderFallbackEditorModal() {
    const editorState = state.fallbackEditor;
    
    let content = '<div class="fallback-editor">';
    
    // Source indicator
    content += '<div class="fallback-source-badge">';
    content += '<span class="source-label">Source:</span> ';
    content += '<span class="source-value ' + editorState.source + '">' + editorState.source + '</span>';
    if (editorState.hasChanges) {
        content += '<span class="changes-indicator">(unsaved changes)</span>';
    }
    content += '</div>';
    
    // Validation error banner
    const errorKeys = Object.keys(editorState.validationErrors);
    const saveError = editorState.validationErrors['_save'];
    const fieldErrors = errorKeys.filter(function(k) { return k !== '_save'; });
    
    if (saveError || fieldErrors.length > 0) {
        content += '<div class="fallback-validation-banner">';
        content += '<span class="validation-icon">⚠️</span> ';
        if (saveError) {
            content += '<span class="save-error">' + escapeHtml(saveError) + '</span>';
        } else {
            content += '<span>Please fix validation errors before saving</span>';
        }
        content += '</div>';
    }
    
    // Entries list
    content += '<div id="fallback-entries-list" class="fallback-entries-list">';
    if (editorState.entries.length === 0) {
        content += '<div class="fallback-empty">No fallback entries configured. Click "Add Entry" to add one.</div>';
    } else {
        editorState.entries.forEach(function(entry, index) {
            content += renderFallbackEntry(index, entry, editorState.validationErrors[index]);
        });
    }
    content += '</div>';
    
    // Action buttons
    content += '<div class="fallback-actions">';
    content += '<button id="fallback-add-entry" class="btn btn-secondary" onclick="fallbackAddEntry()">+ Add Entry</button>';
    content += '<button id="fallback-reset-upstream" class="btn btn-secondary" onclick="fallbackResetUpstream()">↻ Reset to Upstream</button>';
    content += '</div>';
    
    // Footer actions
    content += '<div class="fallback-footer">';
    content += '<button class="btn btn-text" onclick="closeModal()">Cancel</button>';
    const saveDisabled = fieldErrors.length > 0 ? ' disabled' : '';
    content += '<button id="fallback-save" class="btn btn-primary"' + saveDisabled + ' onclick="fallbackSave()">Save Changes</button>';
    content += '</div>';
...
/**
 * Render a single fallback entry row
 */
function renderFallbackEntry(index, entry, validationError) {
    const hasError = validationError && validationError.length > 0;
    
    let html = '<div class="fallback-entry" data-testid="fallback-entry-' + index + '">';
    
    // Entry header with rank and controls
    html += '<div class="fallback-entry-header">';
    html += '<span class="fallback-rank">#' + (index + 1) + '</span>';
    html += '<div class="fallback-entry-controls">';
    html += '<button data-testid="fallback-move-up-' + index + '" class="btn btn-icon" onclick="fallbackMoveUp(' + index + ')"' + (index === 0 ? ' disabled' : '') + ' title="Move up">↑</button>';
    html += '<button data-testid="fallback-move-down-' + index + '" class="btn btn-icon" onclick="fallbackMoveDown(' + index + ')"' + (index === state.fallbackEditor.entries.length - 1 ? ' disabled' : '') + ' title="Move down">↓</button>';
    html += '<button data-testid="fallback-remove-' + index + '" class="btn btn-icon btn-danger" onclick="fallbackRemoveEntry(' + index + ')" title="Remove">✕</button>';
    html += '</div>';
    html += '</div>';
    
    // Entry fields
    html += '<div class="fallback-entry-fields">';
    
    // Model field
    html += '<div class="fallback-field">';
    html += '<label>Model</label>';
    const modelError = validationError && validationError.model ? ' has-error' : '';
    html += '<input type="text" data-testid="fallback-model-' + index + '" class="fallback-input' + modelError + '" value="' + escapeHtml(entry.model) + '" placeholder="e.g., claude-opus-4-6" onchange="fallbackUpdateField(' + index + ', \'model\', this.value)" />';
    if (validationError && validationError.model) {
        html += '<span class="field-error">' + escapeHtml(validationError.model) + '</span>';
    }
    html += '</div>';
    
    // Providers field
    html += '<div class="fallback-field">';
    html += '<label>Providers <span class="field-hint">(comma-separated)</span></label>';
    const providersError = validationError && validationError.providers ? ' has-error' : '';
    html += '<input type="text" data-testid="fallback-providers-' + index + '" class="fallback-input' + providersError + '" value="' + escapeHtml(entry.providers) + '" placeholder="e.g., anthropic, github-copilot" onchange="fallbackUpdateField(' + index + ', \'providers\', this.value)" />';
    if (validationError && validationError.providers) {
        html += '<span class="field-error">' + escapeHtml(validationError.providers) + '</span>';
    }
    html += '</div>';
    
    // Variant field (optional)
    html += '<div class="fallback-field">';
    html += '<label>Variant <span class="field-hint">(optional)</span></label>';
    html += '<input type="text" data-testid="fallback-variant-' + index + '" class="fallback-input" value="' + escapeHtml(entry.variant || '') + '" placeholder="e.g., max" onchange="fallbackUpdateField(' + index + ', \'variant\', this.value)" />';
    html += '</div>';
    
    html += '</div>'; // .fallback-entry-fields
    html += '</div>'; // .fallback-entry
 ...
 */
function fallbackUpdateField(index, field, value) {
    if (index < 0 || index >= state.fallbackEditor.entries.length) return;
    
    state.fallbackEditor.entries[index][field] = value;
    state.fallbackEditor.hasChanges = true;
    
    // Clear any previous save error since user is making changes
    delete state.fallbackEditor.validationErrors['_save'];
    
    // Re-validate this entry
    validateFallbackEntry(index);
    
    // Re-render just to update save button state
    updateSaveButtonState();
}

/**
 * Validate a single entry and update errors
 */
function validateFallbackEntry(index) {
    const entry = state.fallbackEditor.entries[index];
    if (!entry) return;
    ...
    // Providers must have at least one non-empty value
    const providers = entry.providers.split(',').map(function(p) { return p.trim(); }).filter(function(p) { return p !== ''; });
    if (providers.length === 0) {
        errors.providers = 'At least one provider is required';
    }
    
    if (Object.keys(errors).length > 0) {
        state.fallbackEditor.validationErrors[index] = errors;
    } else {
        delete state.fallbackEditor.validationErrors[index];
    }
}
 ...
 */
function validateAllFallbackEntries() {
    state.fallbackEditor.validationErrors = {};
    
    state.fallbackEditor.entries.forEach(function(entry, index) {
        validateFallbackEntry(index);
    });
    
    return Object.keys(state.fallbackEditor.validationErrors).length === 0;
}
...
function updateSaveButtonState() {
    const saveBtn = document.getElementById('fallback-save');
    if (!saveBtn) return;
    
    const hasErrors = Object.keys(state.fallbackEditor.validationErrors).length > 0;
    saveBtn.disabled = hasErrors;
}

/**
 * Move an entry up in the order
 */
function fallbackMoveUp(index) {
    if (index <= 0 || index >= state.fallbackEditor.entries.length) return;
    
    // Swap with previous entry
    const entries = state.fallbackEditor.entries;
    ...
    // Re-validate affected entries
    validateFallbackEntry(index - 1);
    validateFallbackEntry(index);
...
/**
 * Move an entry down in the order
 */
function fallbackMoveDown(index) {
    if (index < 0 || index >= state.fallbackEditor.entries.length - 1) return;
 ...
 */
function fallbackRemoveEntry(index) {
    if (index < 0 || index >= state.fallbackEditor.entries.length) return;
    
    state.fallbackEditor.entries.splice(index, 1);
    delete state.fallbackEditor.validationErrors[index];
    
    // Rebuild validation errors with correct indices
    const newErrors = {};
    Object.keys(state.fallbackEditor.validationErrors).forEach(function(key) {
        const oldIndex = parseInt(key, 10);
        if (oldIndex > index) {
            newErrors[oldIndex - 1] = state.fallbackEditor.validationErrors[oldIndex];
        } else if (oldIndex < index) {
            newErrors[oldIndex] = state.fallbackEditor.validationErrors[oldIndex];
        }
    });
    state.fallbackEditor.validationErrors = newErrors;
 ...
 */
function fallbackAddEntry() {
    state.fallbackEditor.entries.push({
        model: '',
        providers: '',
        variant: ''
    });
    
    // Mark new entry as invalid (empty model/providers)
    const newIndex = state.fallbackEditor.entries.length - 1;
    state.fallbackEditor.validationErrors[newIndex] = {
    ...
    // Focus the model input of the new entry
    setTimeout(function() {
        const modelInput = document.querySelector('[data-testid="fallback-model-' + newIndex + '"]');
        if (modelInput) modelInput.focus();
    }, 0);
}

/**
 * Reset to upstream defaults by re-fetching from API
 */
async function fallbackResetUpstream() {
    if (!confirm('Reset fallback chain to upstream defaults? This will discard your changes.')) {
        return;
    }
    
    const agentName = state.fallbackEditor.agentName;
    
    try {
        // Call DELETE to reset override
        const response = await fetch('/api/agents/' + encodeURIComponent(agentName) + '/fallbacks', {
            method: 'DELETE'
        });
        ...
        // Close modal, show success, reload agent data
        closeModal();
        updateStatus('Fallback chain reset to upstream', 'success');
        
        // Reload agents to reflect the change in recommendations
        await renderAgentConfigView();
    } catch (error) {
        console.error('Failed to reset fallback chain:', error);
        // Keep modal open, show error, preserve local state
        updateStatus('Failed to reset: ' + error.message, 'error');
    }
}

/**
 * Save the fallback chain via PUT API
 */
async function fallbackSave() {
    // Validate all entries
    const isValid = validateAllFallbackEntries();
    ...
    // Build the payload
    const payload = {
        fallbackChain: state.fallbackEditor.entries.map(function(entry) {
            return {
                model: entry.model.trim(),
                providers: entry.providers.split(',').map(function(p) { return p.trim(); }).filter(function(p) { return p !== ''; }),
                variant: entry.variant ? entry.variant.trim() : undefined
            };
        })
    };
    
    // Remove undefined variant values
    payload.fallbackChain.forEach(function(entry) {
        if (!entry.variant) delete entry.variant;
    });
    
    const agentName = state.fallbackEditor.agentName;
    
    try {
        // Call PUT to save the override
        const response = await fetch('/api/agents/' + encodeURIComponent(agentName) + '/fallbacks', {
            method: 'PUT',
            headers: {
        ...
        // Close modal, show success, reload agent data
        closeModal();
        updateStatus('Fallback chain saved', 'success');
        
        // Reload agents to reflect the change in recommendations
        await renderAgentConfigView();
    } catch (error) {
        console.error('Failed to save fallback chain:', error);
        // Keep modal open, show error, preserve local state
        state.fallbackEditor.validationErrors['_save'] = error.message;
        renderFallbackEditorModal();
        updateStatus('Failed to save: ' + error.message, 'error');
    }
}
...
window.openAgentFallbackEditor = openAgentFallbackEditor;
window.fallbackUpdateField = fallbackUpdateField;
window.fallbackMoveUp = fallbackMoveUp;
window.fallbackMoveDown = fallbackMoveDown;
window.fallbackRemoveEntry = fallbackRemoveEntry;
window.fallbackAddEntry = fallbackAddEntry;
window.fallbackResetUpstream = fallbackResetUpstream;
window.fallbackSave = fallbackSave;
