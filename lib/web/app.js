...
function syncUnsavedState() {
    state.unsavedChanges = getPendingModelChanges().length > 0;
...
async function renderAgentConfigView() {
    ...
    try {
        ...
        elements.agentsGrid.innerHTML = agents.map(agentName => {
            ...
            let html = '<div class="agent-config-card' + (hasPendingChange ? ' pending-change' : '') + '">';
...
function assignModelToAgent(modelId, agentName) {
    ...
    if (oldModelId === modelId) {
        updateStatus(agentName + ' is already using that model', 'info');
        return;
    }
