...
function openFallbackEditor(agentName, options) {
    ...
    const currentFallbacks = restoreState
        ? normalizeFallbackModels(restoreState.fallbacks)
        : normalizeFallbackModels(agentConfig.fallback_models);
    const originalFallbacks = restoreState
        ? normalizeFallbackModels(restoreState.originalFallbacks)
        : [...currentFallbacks];
