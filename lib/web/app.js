 ...
 * Configures agent model assignments for oh-my-openagent (upstream repo)
 * Config file: ~/.config/opencode/oh-my-opencode.jsonc
...
function setupEventListeners() {
    ...
    const upstreamSyncBtn = document.getElementById('upstream-sync-btn');
    if (upstreamSyncBtn) {
        upstreamSyncBtn.addEventListener('click', openUpstreamSyncModal);
    }
    ...
    elements.modal.addEventListener('click', (e) => {
        ...
        if (actionBtn && !actionBtn.disabled) {
            ...
            if (action === 'assign-model') {
                ...
                const agentName = actionBtn.getAttribute('data-agent-name');
                const variant = actionBtn.getAttribute('data-variant') || null;
                assignModelToAgent(modelId, agentName, variant);
                closeModal();
            } else if (action === 'assign-recommended') {
                const modelId = actionBtn.getAttribute('data-model-id');
                const agentName = actionBtn.getAttribute('data-agent-name');
                assignModelToAgent(modelId, agentName);
                closeModal();
            } else if (action === 'assign-prompt') {
                const modelId = actionBtn.getAttribute('data-model-id');
                const agentName = actionBtn.getAttribute('data-agent-name');
                assignModelToAgent(modelId, agentName);
                closeModal();
                ...
                closeModal();
            } else if (action === 'prompt-assign') {
                const modelId = actionBtn.getAttribute('data-model-id');
                promptAssignToAgent(modelId);
            }
        }
    });
...
/**
 * Filter models by state criteria (search, provider, capabilities, minContext).
 * Shared utility used by both model selector and fallback picker.
 * @param {Array} models - Array of model objects
 * @param {Object} filterState - Filter state with search, provider, capabilities, minContext
 * @returns {Array} Filtered models array
 */
function filterModelsByState(models, filterState) {
    let filtered = models;
    ...
    if (filterState.provider) {
        filtered = filtered.filter(m => m.provider === filterState.provider);
    }

    if (filterState.capabilities && filterState.capabilities.length > 0) {
        filtered = filtered.filter(model => {
            return filterState.capabilities.every(cap => {
                switch (cap) {
    ...
    if (filterState.minContext) {
        filtered = filtered.filter(m => (m.context || 0) >= filterState.minContext);
    }
...
/**
 * Extract model ID from a fallback entry (string or object with .model)
 */
function extractModelId(entry) {
    if (entry == null) return null;
    if (typeof entry === 'string') {
        const t = entry.trim();
        return (t.length > 0 && t.indexOf('/') > 0 && t.indexOf('/') < t.length - 1) ? t : null;
    }
    if (typeof entry === 'object' && !Array.isArray(entry) && typeof entry.model === 'string') {
        const t = entry.model.trim();
        return (t.length > 0 && t.indexOf('/') > 0 && t.indexOf('/') < t.length - 1) ? t : null;
    }
    return null;
}
 ...
 * Handles strings, objects with { model: "provider/name" }, and mixed arrays.
 ...
 * Strings stay strings, objects keep all properties.
 */
function normalizeFallbackModelsRich(input) {
    if (!input) return [];
...
/**
 * Format a fallback entry as readable label for UI display.
...
function formatFallbackLabel(entry) {
    ...
    if (typeof entry === 'object' && !Array.isArray(entry)) {
        ...
        if (entry.reasoningEffort) badges.push('reasoning:' + entry.reasoningEffort);
        return badges.length > 0 ? model + ' (' + badges.join(', ') + ')' : model;
    }
 ...
 * Handles { providers: [...], model: "...", variant: "..." } shape.
 */
function formatUpstreamFallbackLabel(entry) {
    ...
    if (typeof entry === 'object' && !Array.isArray(entry)) {
        ...
        // Avoid [object Object] for empty/unknown objects
        return JSON.stringify(entry);
    }
    // Fallback: avoid [object Object] for arrays or unexpected types
    if (entry != null && typeof entry === 'object') return JSON.stringify(entry);
    return String(entry);
}
...
function getPendingModelChanges() {
    ...
    return diff.map(change => {
        ...
        const displayModelId = change.model || toModelId || fromModelId;
        ...
        const result = {
            ...
            toDisplay: getModelDisplay(displayModelId)
        };
...
async function showAgentModelSelector(agentName) {
    ...
    function filterModelsForSelector(models) {
        return filterModelsByState(models, selectorState);
    }
    ...
    function renderModelButton(model, extraMeta, recommendation) {
        ...
        const variantAttr = variant ? ' data-variant="' + escapeHtml(variant) + '"' : '';
        let html = '<button class="btn btn-secondary model-select-btn ' + (isCurrent ? 'current' : '') + '" data-model-id="' + escapeHtml(model.id) + '" data-agent-name="' + escapeHtml(agentName) + '"' + (variant ? ' data-variant="' + escapeHtml(variant) + '"' : '') + ' data-action="assign-model"' + variantAttr + '>';
...
function viewModelDetails(modelId) {
    ...
    if (agents.length > 0) {
        ...
        agents.forEach(agent => {
            ...
            content += '<button class="btn btn-small btn-secondary" onclick="viewAlternatives(\'' + escapeHtml(agent) + '\')">Change</button></div>';
        });
        content += '</div></div>';
    }
...
function viewAlternatives(agentName) {
    ...
    if (alternatives.length > 0) {
        alternatives.forEach(alt => {
            ...
            content += '</div>';
            content += '<button class="btn btn-primary" data-action="assign-model" data-model-id="' + escapeHtml(alt.model.id) + '" data-agent-name="' + escapeHtml(agentName) + '">Switch to This</button>';
            content += '</div>';
        });
    } else {
...
function assignModel(modelId) {
    let content = '<p>Select an agent to assign <strong>' + escapeHtml(modelId) + '</strong> to:</p><div class="agent-list">';

    const agents = state.currentConfig && state.currentConfig.agents ? Object.keys(state.currentConfig.agents) : [];

    if (agents.length > 0) {
        agents.forEach(agent => {
            content += '<button class="btn btn-secondary assign-agent-btn" data-action="assign-model" data-model-id="' + escapeHtml(modelId) + '" data-agent-name="' + escapeHtml(agent) + '">' + escapeHtml(agent) + '</button>';
        });
    } else {
...
function openSaveChangesReviewModal() {
    ...
    pendingChanges.forEach(change => {
        ...
        if (change.type === 'changed') {
            ...
            content += '<span class="diff-to">' + change.toDisplay + '</span>';
        } else if (change.type === 'fallback_changed') {
            content += '<span class="diff-to">' + change.toDisplay + '</span>';
...
function getCurrentModelForAgent(agentName) {
    ...
    return agentConfig ? (agentConfig.model || null) : null;
}
...
async function viewAgentDetail(agentName) {
    try {
        ...
        const configuredFallbacksRaw = fullAgent.configuredFallbacksRaw || fullAgent.configuredFallbackModels || [];
        if (configuredFallbacksRaw.length > 0) {
            ...
            configuredFallbacksRaw.forEach(entry => {
                content += '<li><code>' + escapeHtml(formatFallbackLabel(entry)) + '</code></li>';
            });
            content += '</ol>';
        ...
        if (fullAgent.fallbackChain && fullAgent.fallbackChain.length > 0) {
            ...
            fullAgent.fallbackChain.forEach(model => {
                content += '<li><code>' + escapeHtml(formatUpstreamFallbackLabel(model)) + '</code></li>';
            });
            content += '</ol>';
            content += '</div>';
        }
        ...
        if (fullAgent.recommendedModel) {
            ...
            content += '<span>' + escapeHtml(fullAgent.recommendedModel.name) + '</span>';
            content += '<button class="btn btn-primary" data-action="assign-recommended" data-model-id="' + escapeHtml(fullAgent.recommendedModel.id) + '" data-agent-name="' + escapeHtml(agentName) + '">Use This Model</button>';
            content += '</div>';
            content += '</div>';
        }
...
async function compareProviders(modelId) {
    try {
        ...
        data.variants.forEach((variant, index) => {
            ...
            content += '<button class="btn btn-primary" data-action="prompt-assign" data-model-id="' + escapeHtml(variant.model.id) + '">Use This Provider</button>';
            content += '</div>';
...
function promptAssignToAgent(modelId) {
    ...
    agents.forEach(agent => {
        ...
        content += '<button class="btn btn-secondary agent-select-btn" data-action="assign-prompt" data-model-id="' + escapeHtml(modelId) + '" data-agent-name="' + escapeHtml(agent) + '">';
        content += '<div class="agent-select-name">' + escapeHtml(agent) + '</div>';
        content += '<div class="agent-select-current">Currently: ' + escapeHtml(currentModelName) + '</div>';
        content += '</button>';
    });

    content += '</div>';
    content += '<button class="btn btn-text" data-action="close-modal" style="margin-top: var(--spacing-md)">Cancel</button>';
    content += '</div>';
...
function renderProviderDiagnosticsModal(diagnostics) {
    ...
    const configProviders = Array.isArray(fromConfig.providersNormalized)
        ? fromConfig.providersNormalized
        : Object.keys(fromConfig);
    const pluginHints = Array.isArray(fromConfig.pluginHints) ? fromConfig.pluginHints : [];
    const configWarnings = Array.isArray(fromConfig.warnings) ? fromConfig.warnings : [];
    content += '<div class="diagnostics-source-group">';
    content += '<h4>From Config</h4>';
    if (configProviders.length === 0 && pluginHints.length === 0 && configWarnings.length === 0) {
        content += '<p class="diagnostics-empty">No providers configured</p>';
    } else {
        if (configProviders.length > 0) {
            content += '<ul class="diagnostics-list">';
            configProviders.forEach(function(provider) {
                const modelCount = fromConfig[provider];
        ...
        if (pluginHints.length > 0) {
            content += '<div class="diagnostics-subsection"><strong>Plugin Hints</strong>: ' + pluginHints.map(escapeHtml).join(', ') + '</div>';
        }
        if (configWarnings.length > 0) {
            content += '<ul class="diagnostics-list">';
            configWarnings.forEach(function(warning) {
                content += '<li>' + escapeHtml(warning) + '</li>';
            });
            content += '</ul>';
        }
    }
    ...
    const assignmentSources = Array.isArray(fromAssignments.sources) ? fromAssignments.sources : null;
    const assignmentWarnings = Array.isArray(fromAssignments.warnings) ? fromAssignments.warnings : [];
    content += '<div class="diagnostics-source-group">';
    content += '<h4>From Agent Assignments</h4>';
    if ((!assignmentSources || assignmentSources.length === 0) && Object.keys(fromAssignments).length === 0 && assignmentWarnings.length === 0) {
        content += '<p class="diagnostics-empty">No agent assignments</p>';
    } else {
        content += '<ul class="diagnostics-list">';
        if (assignmentSources) {
            assignmentSources.forEach(function(source) {
                content += '<li><span class="provider-name">' + escapeHtml(source.provider) + '</span>';
                if (typeof source.modelRefCount === 'number') {
                    content += ' <span class="agent-count">(' + source.modelRefCount + ' references)</span>';
                }
                content += '</li>';
            });
        } else {
        ...
        assignmentWarnings.forEach(function(warning) {
            content += '<li>' + escapeHtml(warning) + '</li>';
        });
        content += '</ul>';
    }
    ...
    const matchedProviders = mismatches.matched || [];
    ...
    // Matched providers (exact + normalized)
    content += '<div class="mismatch-category">';
    content += '<h4 class="mismatch-title alias">Matched Providers (' + matchedProviders.length + ')</h4>';
    if (matchedProviders.length === 0) {
        content += '<p class="diagnostics-empty">None</p>';
    } else {
        content += '<ul class="diagnostics-list mismatch-list">';
        matchedProviders.forEach(function(item) {
    ...
    // Auth Diagnostics Section
    var auth = diagnostics.auth || null;
    content += '<div class="diagnostics-section">';
    content += '<h3>Auth Diagnostics</h3>';

    if (!auth || !auth.providers || Object.keys(auth.providers).length === 0) {
        content += '<p class="diagnostics-empty">No auth diagnostics available</p>';
    } else {
        var authProviders = auth.providers;
        var authWarnings = Array.isArray(auth.warnings) ? auth.warnings : [];
        var authFileExists = auth.authFile && auth.authFile.exists;

        content += '<div class="auth-diagnostics-grid">';

        var authProviderKeys = Object.keys(authProviders).sort();
        authProviderKeys.forEach(function(providerKey) {
            var providerState = authProviders[providerKey];
            if (!providerState) return;

            var status = providerState.status || 'unknown';
            var statusClass = status === 'present' ? 'auth-present' : (status === 'missing' ? 'auth-missing' : 'auth-unknown');
            var statusIcon = status === 'present' ? '✓' : (status === 'missing' ? '✗' : '?');

            content += '<div class="auth-provider-card ' + statusClass + '">';
            content += '<div class="auth-provider-header">';
            content += '<span class="provider-name">' + escapeHtml(providerKey) + '</span>';
            content += ' <span class="auth-status ' + statusClass + '">' + statusIcon + ' ' + escapeHtml(status) + '</span>';
            content += '</div>';

            // Detected auth types
            var detectedTypes = Array.isArray(providerState.detectedAuthTypes) ? providerState.detectedAuthTypes : [];
            if (detectedTypes.length > 0) {
                content += '<div class="auth-detail">';
                content += '<span class="auth-label">Detected:</span> ';
                content += detectedTypes.map(function(t) {
                    return '<span class="auth-type-badge">' + escapeHtml(t) + '</span>';
                }).join(' ');
                content += '</div>';
            }

            // Sources (metadata only, never secrets)
            var sources = Array.isArray(providerState.sources) ? providerState.sources : [];
            if (sources.length > 0) {
                content += '<div class="auth-sources">';
                sources.forEach(function(source) {
                    content += '<div class="auth-source-item">';
                    content += '<span class="auth-label">Source:</span> ';
                    content += '<span class="auth-source-kind">' + escapeHtml(source.kind || 'unknown') + '</span>';
                    if (source.authType) {
                        content += ' <span class="auth-type-badge">' + escapeHtml(source.authType) + '</span>';
                    }
                    if (Array.isArray(source.fields) && source.fields.length > 0) {
                        content += ' <span class="auth-fields">(' + source.fields.map(escapeHtml).join(', ') + ')</span>';
                    }
                    if (source.env) {
                        content += ' <span class="auth-env">' + escapeHtml(source.env) + '</span>';
                    }
                    if (source.redacted) {
                        content += ' <span class="auth-redacted">🔒 redacted</span>';
                    }
                    content += '</div>';
                });
                content += '</div>';
            ...
            var providerWarnings = Array.isArray(providerState.warnings) ? providerState.warnings : [];
            if (providerWarnings.length > 0) {
                content += '<div class="auth-provider-warnings">';
                providerWarnings.forEach(function(w) {
                    content += '<div class="auth-warning">⚠ ' + escapeHtml(w) + '</div>';
                });
                content += '</div>';
            }

            content += '</div>'; // .auth-provider-card
        });

        content += '</div>'; // .auth-diagnostics-grid

        // Top-level auth warnings
        if (authWarnings.length > 0) {
            content += '<div class="auth-section-warnings">';
            authWarnings.forEach(function(w) {
                content += '<div class="auth-warning">⚠ ' + escapeHtml(w) + '</div>';
            });
            content += '</div>';
        }

        // Auth file status indicator
        if (auth.authFile) {
            content += '<div class="auth-file-status">';
            if (authFileExists) {
                content += '<span class="auth-present">✓ Auth file present</span>';
            } else {
                content += '<span class="auth-missing">✗ No auth file found</span>';
            }
            content += '</div>';
        }

        // Safety notice
        content += '<div class="auth-safety-notice">🔒 Secret values are never displayed in this UI</div>';
    }
    ...
    if (lmStudioPolicy.customDetection === 'disabled' || lmStudioPolicy.customDetection === false) {
 ...
 */
function openFallbackEditor(agentName, options) {
    ...
    const currentFallbacks = restoreState
        ? normalizeFallbackModelsRich(restoreState.fallbacks)
        : normalizeFallbackModelsRich(agentConfig.fallback_models);
    const originalFallbacks = restoreState
        ? normalizeFallbackModelsRich(restoreState.originalFallbacks)
        : [...currentFallbacks];
    ...
    state.fallbackEditorState = {
        ...
        originalFallbacks: [...originalFallbacks]
    };
...
function renderFallbackEditorList(fallbacks) {
    ...
    fallbacks.forEach((entry, index) => {
        const modelId = extractModelId(entry) || '';
        const label = formatFallbackLabel(entry);
        const isObject = typeof entry === 'object' && entry !== null;
        html += '<div class="fallback-item" data-index="' + index + '">';
        html += '<div class="fallback-item-info">';
        html += '<span class="fallback-item-id">' + escapeHtml(label) + '</span>';
        html += '<span class="fallback-item-priority">Priority ' + (index + 1) + '</span>';
        if (isObject) {
            html += '<span class="fallback-item-badge" title="Object entry with preserved fields">obj</span>';
        }
        html += '</div>';
...
function addFallbackEntry() {
    ...
    if (!isProviderModelId(value)) {
        errorDiv.textContent = 'Invalid format. Use provider/model (e.g., anthropic/claude-sonnet-4-6, openai/gpt-5.4, google/gemini-3.1-pro)';
        errorDiv.classList.remove('hidden');
        return;
    }

    // Check for duplicates
    if (state.fallbackEditorState.fallbacks.some(e => extractModelId(e) === value)) {
        errorDiv.textContent = `"${value}" is already in the fallback list`;
        errorDiv.classList.remove('hidden');
        return;
    }
...
function showFallbackModelPicker() {
    ...
    if (currentFallbacks.length > 0) {
        content += '<div class="model-selector-current">Current fallbacks: <code>' + currentFallbacks.map(e => escapeHtml(formatFallbackLabel(e))).join('</code>, <code>') + '</code></div>';
    }
    content += '</div>';
    ...
    const pickerState = {
        ...
        minContext: null,
        capabilities: [],
        renderResults: null  // Will be set below
    };

    // Store render function reference in state for external access
    state.fallbackEditorState.renderPickerResults = function() {
        ...
        if (filtered.length === 0) {
            ...
            const alreadyAdded = filtered.filter(m => currentFallbacks.some(e => extractModelId(e) === m.id));
            const available = filtered.filter(m => !currentFallbacks.some(e => extractModelId(e) === m.id));
    ...
    function filterModelsForPicker(models) {
        return filterModelsByState(models, pickerState);
    }

    function renderPickerModelButton(model) {
        const isDuplicate = currentFallbacks.some(e => extractModelId(e) === model.id);
        ...
        if (isDuplicate) {
            ...
            html += ' data-action="select-fallback" data-model-id="' + escapeHtml(model.id) + '"';
        }
    ...
    function renderPickerResults() {
        state.fallbackEditorState.renderPickerResults();
    }
    ...
    // Delegated click handler for fallback selection buttons
    const fallbackResultsEl = document.getElementById('fallback-model-results');
    if (fallbackResultsEl) {
        fallbackResultsEl.addEventListener('click', function(e) {
            const btn = e.target.closest('[data-action="select-fallback"]');
            if (!btn || btn.disabled) return;
            const modelId = btn.getAttribute('data-model-id');
            selectModelForFallback(modelId);
        });
    }
}
...
function selectModelForFallback(modelId) {
    ...
    if (state.fallbackEditorState.fallbacks.some(e => extractModelId(e) === modelId)) {
    ...
    const restoreState = {
        fallbacks: [...state.fallbackEditorState.fallbacks],
        originalFallbacks: [...state.fallbackEditorState.originalFallbacks]
    };
    ...
    // Safety check: ensure state still exists when timeout fires
    setTimeout(() => {
        if (state.fallbackEditorState && state.fallbackEditorState.agentName) {
            openFallbackEditor(state.fallbackEditorState.agentName, { restoreState: restoreState });
        }
    }, 50);
}
...
/**
 * Open the upstream sync modal to check for and apply updates
 */
async function openUpstreamSyncModal() {
    showModal('Upstream Sync', `
        <div class="upstream-sync-container">
            <div id="upstream-sync-status" class="sync-status">
                <span class="loading">Checking for upstream changes...</span>
            </div>
            <div id="upstream-sync-actions" class="sync-actions hidden">
                <button id="preview-sync-btn" class="btn btn-secondary">Preview Changes</button>
                <button id="apply-sync-btn" class="btn btn-primary">Apply Changes</button>
                <button id="cancel-sync-btn" class="btn btn-text">Cancel</button>
            </div>
            <div id="upstream-sync-results" class="sync-results"></div>
        </div>
    `);

    // Check for drift
    try {
        const response = await fetch('/api/upstream/drift');
        const data = await response.json();

        const statusEl = document.getElementById('upstream-sync-status');
        const actionsEl = document.getElementById('upstream-sync-actions');
        const resultsEl = document.getElementById('upstream-sync-results');

        if (data.error && !data.upstreamResolved) {
            statusEl.innerHTML = `<span class="error">⚠️ ${data.error}</span>`;
            return;
        }

        if (!data.hasDrift) {
            statusEl.innerHTML = `<span class="success">✅ Up to date! No changes needed.</span>`;
            if (data.pinnedSha) {
                statusEl.innerHTML += `<br><span class="sha-info">Pinned: ${data.pinnedSha.slice(0, 8)}</span>`;
            }
            return;
        }

        // Show drift info
        statusEl.innerHTML = `<span class="warning">⚠️ Upstream changes detected</span>`;

        let html = '<div class="drift-details">';

        if (data.changedAgents && data.changedAgents.length > 0) {
            html += `<h4>Changed Agents (${data.changedAgents.length})</h4><ul>`;
            for (const change of data.changedAgents) {
                html += `<li><strong>${change.name}</strong>: ${change.upstream?.fallbackChain?.length || 0} entries upstream vs ${change.local?.fallbackChain?.length || 0} local</li>`;
            }
            html += '</ul>';
        }

        if (data.newAgents && data.newAgents.length > 0) {
            html += `<h4>New Agents (${data.newAgents.length})</h4><ul>`;
            for (const agent of data.newAgents) {
                html += `<li>${agent}</li>`;
            }
            html += '</ul>';
        }
        ...
        if (data.actionRequired && data.actionRequired.length > 0) {
            html += `<h4>Actions Required</h4><ul>`;
            for (const action of data.actionRequired) {
                html += `<li>${action}</li>`;
            }
            html += '</ul>';
        }
        ...
        // Bind button events
        document.getElementById('preview-sync-btn').addEventListener('click', () => performSync(true));
        document.getElementById('apply-sync-btn').addEventListener('click', () => performSync(false));
        document.getElementById('cancel-sync-btn').addEventListener('click', closeModal);

    } catch (error) {
        document.getElementById('upstream-sync-status').innerHTML =
            `<span class="error">❌ Error checking drift: ${error.message}</span>`;
    }
}
 ...
 */
async function performSync(dryRun) {
    const statusEl = document.getElementById('upstream-sync-status');
    const actionsEl = document.getElementById('upstream-sync-actions');
    const resultsEl = document.getElementById('upstream-sync-results');

    statusEl.innerHTML = dryRun ?
        '<span class="loading">Previewing changes...</span>' :
        '<span class="loading">Applying changes...</span>';
    actionsEl.classList.add('hidden');

    try {
        const response = await fetch('/api/upstream/sync', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.error) {
            statusEl.innerHTML = `<span class="error">❌ Sync failed: ${data.error}</span>`;
            actionsEl.classList.remove('hidden');
            return;
        }

        if (dryRun) {
            statusEl.innerHTML = '<span class="success">✅ Preview complete</span>';
            resultsEl.innerHTML += `<div class="preview-output"><pre>${data.output || 'Changes would be applied'}</pre></div>`;
            actionsEl.classList.remove('hidden');
        } else {
            statusEl.innerHTML = '<span class="success">✅ Changes applied successfully!</span>';
            resultsEl.innerHTML += `<div class="success-details">Updated ${data.changes?.length || 0} agent(s)</div>`;

            // Reload data to reflect changes
            setTimeout(() => {
                loadData();
                updateStatus('Model requirements synced with upstream', 'success');
            }, 1000);
        }
    } catch (error) {
...
window.openUpstreamSyncModal = openUpstreamSyncModal;
