...
/**
 * Calculate diff between two assignment sets for use when switching profiles.
 * Each assignment is an object with { model: string, fallback_models: string[] }
 */
function calculateDiff(current, newAssignments) {
    ...
    allAgents.forEach(agent => {
        ...
        const currentFallback = currentAssign ? currentAssign.fallback_models : [];
        const newFallback = newAssign ? newAssign.fallback_models : [];
 ...
 * Each assignment is now an object with { model: string, fallback_models: string[] }
 */
function calculateDiff(current, newAssignments) {
    const diff = [];
    const allAgents = new Set([...Object.keys(current), ...Object.keys(newAssignments)]);
    ...
    allAgents.forEach(agent => {
        ...
        if (currentModel && newModel && modelChanged) {
            diff.push({ type: 'changed', agent, from: currentModel, to: newModel, fallbackDiff: fallbackDiff.hasChanges ? fallbackDiff : null });
        } else if (!currentModel && newModel) {
            diff.push({ 
                type: 'added', 
                agent, 
        ...
        } else if (fallbackDiff.hasChanges && currentModel && newModel) {
            diff.push({
                type: 'fallback_changed',
                agent,
                model: currentModel,
