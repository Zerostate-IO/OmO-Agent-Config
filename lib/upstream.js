...
function extractAgentsFromSchema(schema) {
  ...
  if (!agentDef || !agentDef.enum || !Array.isArray(agentDef.enum)) {
    return [];
  }
...
function extractSkillsFromSchema(schema) {
  ...
  if (!skillDef || !skillDef.enum || !Array.isArray(skillDef.enum)) {
    return [];
  }
...
/**
 * Validate schema structure and definitions
 * @param {Object} schema - Parsed schema object
 * @returns {Object} Validation result with valid, boolean, definitions, array of errors
 */
function validateSchema(schema) {
  const errors = [];
  const missingDefinitions = [];

  // Check schema is a valid object
  if (!schema || typeof schema !== 'object' || Array.isArray(schema)) {
  ...
  if (!schema.definitions || typeof schema.definitions !== 'object') {
    errors.push('Schema missing definitions object');
    return {
      valid: false,
      missingDefinitions: [],
      errors
  ...
  // If all required definitions exist, the valid
  if (missingDefinitions.length === 0 && errors.length === 0) {
    return {
      valid: true,
      missingDefinitions: [],
      errors: []
