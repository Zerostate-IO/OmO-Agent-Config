...
// Agent fallback overrides file path
const AGENT_FALLBACK_OVERRIDES_FILE = path.join(CONFIG_DIR, 'agent-fallback-overrides.json');

/**
 * Validate a single fallback entry
 * @param {Object} entry - Fallback entry to validate
 * @returns {{valid: boolean, error?: string}} Validation result
 */
function validateFallbackEntry(entry) {
  if (!entry || typeof entry !== 'object') {
    return { valid: false, error: 'Entry must be an object' };
  }
  
  // providers: must be non-empty array of strings
  if (!Array.isArray(entry.providers) || entry.providers.length === 0) {
    return { valid: false, error: 'providers must be a non-empty array' };
  }
  
  for (const provider of entry.providers) {
    if (typeof provider !== 'string' || provider.trim() === '') {
      return { valid: false, error: 'providers must contain non-empty strings' };
    }
  }
  ...
  if ('variant' in entry && (typeof entry.variant !== 'string' || entry.variant.trim() === '')) {
    return { valid: false, error: 'variant must be a non-empty string if provided' };
  }
...
/**
 * Validate a fallback chain array
 * @param {Array} chain - Fallback chain to validate
 * @returns {{valid: boolean, errors: string[]}} Validation result with all errors
 */
function validateFallbackChain(chain) {
  const errors = [];
  
  if (!Array.isArray(chain)) {
    return { valid: false, errors: ['fallbackChain must be an array'] };
  }
  
  chain.forEach((entry, index) => {
    const result = validateFallbackEntry(entry);
    if (!result.valid) {
      errors.push(`fallbackChain[${index}]: ${result.error}`);
    }
  });
...
// In-memory cache for agent fallback overrides
let _agentFallbackOverridesCache = null;
let _agentFallbackOverridesMeta = {
  source: 'default',
  file: null,
  exists: false
};
...
/**
 * Get agent fallback overrides (cached or loaded from file)
 * @returns {Object} Object with version, updatedAt, and agents map
 */
function getAgentFallbackOverrides() {
  // Return cached result if available
  if (_agentFallbackOverridesCache !== null) {
    return _agentFallbackOverridesCache;
  }
  ...
  try {
    if (fs.existsSync(AGENT_FALLBACK_OVERRIDES_FILE)) {
      const content = fs.readFileSync(AGENT_FALLBACK_OVERRIDES_FILE, 'utf8');
      ...
      if (data && typeof data === 'object') {
        ...
        if ('version' in data && data.version !== 1) {
          console.warn('Warning: agent-fallback-overrides.json has unsupported version, using defaults');
          _agentFallbackOverridesCache = defaultEnvelope;
          _agentFallbackOverridesMeta = { source: 'default', file: AGENT_FALLBACK_OVERRIDES_FILE, exists: true };
        ...
        if ('agents' in data && typeof data.agents === 'object') {
          ...
          for (const [agentName, agentData] of Object.entries(data.agents)) {
            ...
            if (!Array.isArray(agentData.fallbackChain)) {
              console.warn(`Warning: agent '${agentName}' has invalid fallbackChain, skipping`);
              continue;
            }
            
            const validation = validateFallbackChain(agentData.fallbackChain);
            if (!validation.valid) {
              console.warn(`Warning: agent '${agentName}' has invalid entries: ${validation.errors.join(', ')}`);
              continue;
            }
            
            validAgents[agentName] = {
              fallbackChain: agentData.fallbackChain
            };
          }
          ...
          _agentFallbackOverridesCache = envelope;
          _agentFallbackOverridesMeta = { source: 'override', file: AGENT_FALLBACK_OVERRIDES_FILE, exists: true };
          return envelope;
        }
      }
    }
    ...
    if (error.code === 'ENOENT') {
      ...
      console.warn('Warning: agent-fallback-overrides.json contains invalid JSON, using defaults');
    } else {
      console.warn(`Warning: Failed to load agent-fallback-overrides.json: ${error.message}, using defaults`);
    }
  }
  
  // Return defaults
  _agentFallbackOverridesCache = defaultEnvelope;
  _agentFallbackOverridesMeta = { source: 'default', file: AGENT_FALLBACK_OVERRIDES_FILE, exists: false };
  return defaultEnvelope;
}

/**
 * Save agent fallback overrides to file
 * @param {Object} overrides - Override envelope { version, updatedAt, agents }
 * @returns {{success: boolean, error?: string}} Result object
 */
function saveAgentFallbackOverrides(overrides) {
  // Validate envelope structure
  if (!overrides || typeof overrides !== 'object') {
    return { success: false, error: 'Overrides must be an object' };
  }
  
  // Validate version
  if ('version' in overrides && overrides.version !== 1) {
    return { success: false, error: 'Only version 1 is supported' };
  }
  
  // Validate agents
  if ('agents' in overrides && typeof overrides.agents === 'object') {
    for (const [agentName, agentData] of Object.entries(overrides.agents)) {
      if (!agentData || typeof agentData !== 'object') {
        return { success: false, error: `Agent '${agentName}' must be an object` };
      }
      
      if (!Array.isArray(agentData.fallbackChain)) {
        return { success: false, error: `Agent '${agentName}' must have a fallbackChain array` };
      }
      
      const validation = validateFallbackChain(agentData.fallbackChain);
      if (!validation.valid) {
        return { success: false, error: `Agent '${agentName}': ${validation.errors.join('; ')}` };
      }
    }
  }
  ...
  try {
    ...
    fs.writeFileSync(AGENT_FALLBACK_OVERRIDES_FILE, JSON.stringify(envelope, null, 2), 'utf8');
...
/**
 * Reset (remove) override for a specific agent
 * @param {string} agentName - Name of the agent to reset
 * @returns {{success: boolean, error?: string}} Result object
 */
function resetAgentFallbackOverride(agentName) {
  if (!agentName || typeof agentName !== 'string') {
    return { success: false, error: 'Agent name must be a non-empty string' };
  }
  
  const current = getAgentFallbackOverrides();
  
  if (!current.agents[agentName]) {
    // Agent not in overrides, nothing to do
    return { success: true };
  }
  
  // Remove the agent
  const updatedAgents = { ...current.agents };
  delete updatedAgents[agentName];
  
  // Save updated envelope
  return saveAgentFallbackOverrides({
...
/**
 * Invalidate the in-memory cache for agent fallback overrides
 */
function invalidateAgentFallbackOverridesCache() {
  _agentFallbackOverridesCache = null;
  _agentFallbackOverridesMeta = {
    source: 'default',
    file: null,
    exists: false
  };
