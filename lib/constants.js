...
const OPENCODE_AUTH_FILE = path.join(process.env.HOME, '.local', 'share', 'opencode', 'auth.json');
const ACTIVE_CONFIG_FILE = path.join(CONFIG_DIR, 'active-config.json');

// Default agent configurations
// Based on Oh My OpenAgent v3.x defaults: https://github.com/code-yeongyu/oh-my-openagent
// Model assignments aligned with upstream first fallback entries (synced to SHA 0904e237)
const DEFAULTS = {
  "agents": {
    "oracle": {
...
const PROVIDER_ALIASES = {
  ...
  // Bailian Coding Plan (Alibaba/Model Studio)
  'bailian-coding-plan': 'bailian-coding-plan',
  'bailian': 'bailian-coding-plan',
  'model-studio': 'bailian-coding-plan',
...
// See: https://github.com/code-yeongyu/oh-my-openagent/blob/dev/src/agents/AGENTS.md
...
,
  "sisyphus-junior": {
    description: "Focused executor for straightforward tasks: execute directly without overhead.",
    access: "write",
    usage: [
      "Use for: single-step or simple multi-step tasks that don't need full orchestration",
      "Best with: capable but cost-efficient models"
    ],
    caveats: [
      "Intended for lower-overhead execution of well-defined tasks."
    ],
    preferred: ["reasoning", "fast"],
    minContext: 64000
  }
};
...
const PROVIDER_POLICIES_DEFAULTS = {
  ...
  'deepseek': {
    ...
    speedTier: 'fast',
    priorityTier: 3,
    notes: 'DeepSeek direct provider models'
  },
  'nvidia': {
    billingModel: 'metered',
    speedTier: 'normal',
    priorityTier: 5,
    notes: 'NVIDIA NIM hosted models'
  },
  'cerebras': {
    billingModel: 'metered',
    speedTier: 'fast',
  ...
  'vercel': {
    ...
    speedTier: 'normal',
    priorityTier: 6,
    notes: 'Vercel AI gateway provider'
  },
  'opencode-go': {
    billingModel: 'unknown',
    speedTier: 'normal',
    priorityTier: 7,
    notes: 'OpenCode Go SDK provider'
  }
};
