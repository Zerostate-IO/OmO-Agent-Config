...
### Fixed
- Synced upstream model requirements mirror (removed `k2p5` entries; aligned `gpt-5.3-codex` provider availability).
- Hephaestus provider caveat now matches upstream `gpt-5.3-codex` availability.
- Thinking detection now treats boolean `capabilities.interleaved` and `variants.*.thinking` as thinking-capable.

### Changed
- Pinned upstream model-requirements commit SHA in `.omo-upstream-sha` and `lib/core/model-requirements.js`.
